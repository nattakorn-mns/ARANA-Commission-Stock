...
async function audRenderStock(body) {
  ...
  const rows = [
    ...
    ...(stockLogs || []).map(r => ({...r, rowType:'stock', itemCount:1, customerName:'-', programSummary:r.productName || '-'}))
  ];
  ...
  <div class="glass-card" style="padding:0;overflow:hidden;"><div class="table-wrap"><table><thead><tr><th>วันที่/เวลา</th><th>สาขา</th><th>ประเภท</th><th>ลูกค้า/รายการ</th><th>โปรแกรม</th><th>จำนวน</th><th>ผู้บันทึก</th><th>หลักฐาน</th><th>สถานะ</th><th>จัดการ</th></tr></thead><tbody>
  ${rows.length ? rows.map(r => `<tr id="stock-audit-row-${r.rowType}-${r.rowType==='opd'?r.billId:r.id}"><td class="nowrap">${formatDate(r.date)}</td><td>${r.branch||'-'}</td><td>${r.rowType==='opd' ? '<span class="badge badge-service">เบิกจาก OPD</span>' : `<span class="badge badge-${r.type==='IN'?'in':'out'}">${r.type==='IN'?'รับของเข้า':'เบิกใช้งาน'}</span>`}</td><td style="font-weight:600;">${r.customerName||'-'}</td><td>${r.programSummary||'-'}</td><td>${r.itemCount} รายการ</td><td>${r.createdByName||'-'}</td><td><button class="btn btn-ghost btn-sm" onclick="${r.rowType==='opd'?`audOpenStockRequest('${r.billId}')`:`audOpenStockLog('${r.id}')`}"><i data-lucide="eye"></i> ดูรายละเอียด</button></td><td>${statusBadge(r.auditStatus||'รอตรวจสอบ')}</td><td><button class="btn btn-success btn-sm" onclick="${r.rowType==='opd'?`audStockAction('${r.billId}','อนุมัติแล้ว')`:`audStockLogAction('${r.id}','อนุมัติแล้ว')`}">อนุมัติ</button> <button class="btn btn-danger btn-sm" onclick="${r.rowType==='opd'?`audStockReject('${r.billId}')`:`audStockLogReject('${r.id}')`}">ตีกลับ</button></td></tr>`).join('') : `<tr><td colspan="10"><div class="empty-state">ไม่มีรายการสต๊อกรอตรวจ</div></td></tr>`}
  </tbody></table></div></div>`;
  lucide.createIcons();
}
