...
async function audRenderStock(body) {
  body.innerHTML = `<div style="padding:24px;text-align:center;color:var(--gray-400);">กำลังโหลด...</div>`;
  const [opdRequests, stockLogs] = await Promise.all([
    DB.getPendingOpdStockRequestsSupabase(),
    DB.getPendingStockLogsSupabase()
  ]);
  window._auditPendingStockLogs = stockLogs || [];
  const grouped = Object.values((stockLogs || []).reduce((acc, log) => {
    const key = log.requestId || log.id;
    (acc[key] ||= { ...log, requestId: key, rowType: 'stock', items: [] }).items.push(log);
    return acc;
  }, {}));
  const rows = [
    ...(opdRequests || []).map(r => ({...r, rowType:'opd', itemCount:r.supplyCount || 0})),
    ...grouped.map(r => ({...r, itemCount:r.items.length, customerName:'-', productSummary:r.items.map(i=>i.productName).join(', ')}))
  ];
  body.innerHTML = `<div class="glass-card" style="margin-bottom:12px;padding:14px 16px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;"><span style="font-size:0.9rem;color:var(--gray-700);font-weight:700;">รายการสต๊อกรอตรวจทั้งหมด</span><span class="badge badge-pending" style="font-size:1rem;padding:4px 12px;">${rows.length} รายการ</span><span style="font-size:0.8rem;color:var(--gray-500);">OPD ${opdRequests?.length||0} ใบ · เบิก/รับทั่วไป ${stockLogs?.length||0} รายการ</span></div>
  ...
  ${rows.length ? rows.map(r => `<tr id="stock-audit-row-${r.rowType}-${r.rowType==='opd'?r.billId:r.requestId}"><td class="nowrap">${formatDate(r.date)}${r.rowType==='stock'&&r.createdAt?`<br><small>${new Date(r.createdAt).toLocaleTimeString('th-TH',{hour:'2-digit',minute:'2-digit'})}</small>`:''}</td><td>${r.branch||'-'}</td><td>${r.rowType==='opd' ? '<span class="badge badge-service">เบิกจาก OPD</span>' : `<span class="badge badge-${r.type==='IN'?'in':'out'}">${r.type==='IN'?'รับของเข้า':r.type==='TRANSFER'?'โอนข้ามสาขา':'เบิกใช้งาน'}</span>`}</td><td style="font-weight:600;">${r.customerName||'คำขอเบิก/รับ'}</td><td>${r.itemCount} รายการ</td><td>${r.createdByName||'-'}</td><td><button class="btn btn-ghost btn-sm" onclick="${r.rowType==='opd'?`audOpenStockRequest('${r.billId}')`:`audOpenStockRequestGroup('${r.requestId}')`}"><i data-lucide="eye"></i> ดูรายละเอียด</button></td><td>${statusBadge(r.auditStatus||'รอตรวจสอบ')}</td><td><button class="btn btn-success btn-sm" onclick="${r.rowType==='opd'?`audStockAction('${r.billId}','อนุมัติแล้ว')`:`audStockRequestAction('${r.requestId}','อนุมัติแล้ว')`}">อนุมัติ</button> <button class="btn btn-danger btn-sm" onclick="${r.rowType==='opd'?`audStockReject('${r.billId}')`:`audStockRequestReject('${r.requestId}')`}">ตีกลับ</button></td></tr>`).join('') : `<tr><td colspan="9"><div class="empty-state">ไม่มีรายการสต๊อกรอตรวจ</div></td></tr>`}
  </tbody></table></div></div>`;
  lucide.createIcons();
}
...
function audOpenStockLog(logId) {
  const log=(window._auditPendingStockLogs||[]).find(x=>x.id===logId); if(!log){Toast.show('ไม่พบรายการสต๊อก','error');return;}
...
function audOpenStockRequestGroup(requestId){const items=(window._auditPendingStockLogs||[]).filter(x=>(x.requestId||x.id)===requestId);if(!items.length){Toast.show('ไม่พบคำขอ','error');return;}const first=items[0];const list=items.map(i=>`<tr><td>${i.productCode||'-'}</td><td>${i.productName||'-'}</td><td>${i.qty} ${i.unit||''}</td><td>${i.source||'-'}</td><td>${i.note||'-'}</td></tr>`).join('');openModal(`<div class="modal" style="width:1100px;max-width:96vw;"><div class="modal-header"><h3 class="modal-title">รายละเอียดคำขอเบิก/รับ</h3><button class="modal-close btn btn-ghost btn-icon btn-sm" onclick="closeModalDirect()">×</button></div><div class="modal-body"><div class="detail-grid"><div><small>วันที่</small><strong>${formatDate(first.date)}</strong></div><div><small>สาขา</small><strong>${first.branch||'-'}</strong></div><div><small>ผู้บันทึก</small><strong>${first.createdByName||'-'}</strong></div><div><small>ประเภท</small><strong>${first.type==='IN'?'รับของเข้า':first.type==='TRANSFER'?'โอนข้ามสาขา':'เบิกใช้งาน'}</strong></div></div><h4 style="margin-top:18px;">รายการทั้งหมด (${items.length})</h4><div class="table-wrap"><table><thead><tr><th>รหัส</th><th>รายการ</th><th>จำนวน</th><th>แหล่งที่มา</th><th>หมายเหตุ</th></tr></thead><tbody>${list}</tbody></table></div></div><div class="modal-footer"><button class="btn btn-danger" onclick="audStockRequestReject('${requestId}')">ตีกลับทั้งคำขอ</button><button class="btn btn-success" onclick="audStockRequestAction('${requestId}','อนุมัติแล้ว')">อนุมัติทั้งคำขอ</button></div></div>`);}
function audStockRequestAction(requestId,status,note){DB.auditStockRequestSupabase(requestId,status,currentUser.id,note||'').then(()=>{Toast.show(status==='อนุมัติแล้ว'?'อนุมัติคำขอแล้ว':'ตีกลับคำขอแล้ว',status==='อนุมัติแล้ว'?'success':'error');closeModalDirect();audRender();}).catch(e=>Toast.show('เกิดข้อผิดพลาด: '+e.message,'error'));}
function audStockRequestReject(requestId){openModal(`<div class="modal"><div class="modal-header"><h3 class="modal-title">ตีกลับคำขอเบิก/รับ</h3></div><div class="modal-body"><label class="form-label">เหตุผลที่ตีกลับ</label><textarea id="aud-stock-request-note" class="form-textarea" rows="4"></textarea></div><div class="modal-footer"><button class="btn btn-ghost" onclick="closeModalDirect()">ยกเลิก</button><button class="btn btn-danger" onclick="audDoStockRequestReject('${requestId}')">ยืนยันตีกลับทั้งคำขอ</button></div></div>`);}
function audDoStockRequestReject(requestId){const n=document.getElementById('aud-stock-request-note')?.value.trim();if(!n){Toast.show('กรุณาระบุเหตุผล','error');return;}audStockRequestAction(requestId,'ตีกลับ',n);}
