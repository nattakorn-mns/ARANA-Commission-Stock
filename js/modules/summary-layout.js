...
function sumRows() {
  const bills = histGetBills(), services = DB._get('bill_services') || [], sales = DB._get('bill_sales') || [], rows = [];
  bills.forEach(b => {
    const sv = services.filter(x => x.billId === b.id && !x.is_superseded && (currentUser.role !== 'Frontdesk' || x.createdBy === currentUser.id));
    const sl = sales.filter(x => x.billId === b.id && !x.is_superseded && (currentUser.role !== 'Frontdesk' || x.createdBy === currentUser.id));
